...
def main():
    ...
    for week, links in week_with_links.items():
        video_ids = []
        sub_urls = []
        for link in links:
        ...
        week_video_ids[week] = video_ids
        week_sub_urls[week] = sub_urls

    week_video_urls = {}
    for week, video_ids in week_video_ids.items():
        video_urls = ['http://youtube.com/watch?v=' + v_id
                      for v_id in video_ids]
        week_video_urls[week] = video_urls

    if len(week_video_urls) < 1:
    ...
    # Download Videos
    for week, video_urls in week_video_urls.items():
        sub_urls = week_sub_urls[week]
        c = 0
        for v, s in zip(video_urls, sub_urls):
            c += 1
            target_dir = os.path.join(args.output_dir,
                                      directory_name(selected_course[0]), week)
            filename_prefix = str(c).zfill(2)
            file_to_save = os.path.join(target_dir, filename_prefix + "-%(title)s.%(ext)s")
            cmd = ["youtube-dl",
